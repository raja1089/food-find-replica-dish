...
async function importData() {
  try {
    console.log("🔄 Importing data from PostgreSQL to MySQL...");

    // Import admin user
    const hashedPassword = "$2b$10$j7F.IRz.GtQizDEWuPKE.OKAnr6Yxo4U55ojgmp8rEPAq1j8sKGKy"; // From PostgreSQL
    ...
    const insertedCities = await db.insert(cities).values(citiesData).onDuplicateKeyUpdate({ set: { name: "name" } });
    ...
    await db.insert(restaurants).values(restaurantsData).onDuplicateKeyUpdate({ set: { name: "name" } });
    ...
    await db.insert(features).values(featuresData).onDuplicateKeyUpdate({ set: { title: "title" } });
    ...
    await db.insert(stats).values({
      ...
      users: 50000000,
      orders: 1000000000
    }).onDuplicateKeyUpdate({ set: { restaurants: 25000 } });
    ...
    await db.insert(footerPages).values(footerPagesData).onDuplicateKeyUpdate({ set: { title: "title" } });
    ...
    await db.insert(heroSection).values(heroSectionData).onDuplicateKeyUpdate({ set: { title: heroSectionData.title } });

    console.log("✅ Data imported successfully to MySQL!");
    console.log("🔑 Admin credentials: username=admin, password=admin123");

  } catch (error) {
    console.error("❌ Error importing data to MySQL:", error);
  }
}
